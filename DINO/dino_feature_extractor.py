...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Facilitate ViT Descriptor extraction."
    )
    parser.add_argument(
        "--image_dir", type=str, required=True, help="path of the extracted image."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="directory to store extracted descriptors (same name as image; '.pt' extension).",
    )
    parser.add_argument(
    ...
    with torch.no_grad():
        ...
        imgfiles = natsorted(glob.glob(os.path.join(args.image_dir, "frame*.png")))
        if len(imgfiles) == 0:
            imgfiles = natsorted(glob.glob(os.path.join(args.image_dir, "*.jpg")))

        os.makedirs(args.output_dir, exist_ok=True)

        for idx in trange(len(imgfiles)):
            imgfile = imgfiles[idx]
            filestem = Path(imgfile).stem
            outfile = os.path.join(args.output_dir, f"{filestem}.pt")

            image_batch, image_pil = extractor.preprocess(imgfile, args.load_size)
            # print(
            #     f"Image {imgfile} is preprocessed to tensor of size {image_batch.shape}."
            # )
            descriptors = extractor.extract_descriptors(
                image_batch.to(device), args.layer, args.facet, args.bin
            )
            # descriptors = descriptors[0, 0, 1:, :]  # (1, 1, H*W+1, 384) -> (H*W, 384)
            descriptors = descriptors.reshape(
                extractor.num_patches[0],
                extractor.num_patches[1],
                descriptors.shape[-1],
            )
            descriptors = descriptors.permute(2, 0, 1)
            descriptors = descriptors.unsqueeze(0)
            if idx == 0:
                print(f"Descriptors are of size: {descriptors.shape}")
            descriptors.detach().cpu().numpy()
            torch.save(descriptors, outfile)
            # print(f"Descriptors saved to: {outfile}")
            # break
