...
def track_vos(model, video, hp=None, mask_enable=False, refine_enable=False, mot_enable=False, device='cpu'):
    ...
    if args.save_mask:
        ...
        for i in range(pred_mask_final.shape[0]):
            # 获取当前帧的掩码
            mask = pred_mask_final[i].astype(np.uint8)
            # 将掩码转换为 CV_8U 格式，避免 cv::imwrite 警告
            mask_8u = cv2.convertScaleAbs(mask)
            # 构建保存掩码图像的文件名
            file_name = join(video_path, image_files[i].split('/')[-1].split('.')[0] + '.png')
            # 保存转换格式后的掩码图像
            cv2.imwrite(file_name, mask_8u)
